...
<section >
	<div class="container-fluid sing-in-form bg-background" >
					...
					<h2 class="sign-in text-center" style="color:#459c09;margin-bottom: 40px;">Registration</h2>
						<div class="col-md-12 col-sm-12 col-xs-12" style="padding:0">
							<?php echo validation_errors(); ?>
								<?php  echo form_open_multipart(base_url('registration')); ?>
									  ...
									  <div class="form-group"  >
											<!--<label for="unique_id">Sponsor ID  <span class="required"> * </span></label>-->
											 <input type="text" placeholder="Sponsor ID"  required name="unique_id" value="<?php if($uniqueid!=''){ echo $uniqueid; }else{ echo set_value('unique_id'); } ?>" class="form-control" id="unique_id"  >
										 </div>   
										  <div class="form-group">
											<!--<label for="email">Select Side  * </label>-->
											<select class="form-control" name="tree_side" > 
												<option <?php if($side=='left'){ echo 'selected'; } ?> value="left">Left Side</option>
												<option <?php if($side=='right'){ echo 'selected'; } ?> value="right" >Right Side</option>
											</select>
										  </div> 
										  <div class="form-group col-sm-12 col-sm-12 col-xs-12 ">
										<div class="register text-center">
										 <button type="submit" class="btn-reply margin-zero btn-login" id="submit_reg_bt" name="register" value="register" style="margin-bottom:20px;font-size:16px">Register</button>	  
										</div>
										</div>	  
									  
										<?php }else { ?>
											<div class="form-group" id="sponser_id_div"> 
												 <input type="text" required placeholder="Desired Member ID"  name="unique_id" value="" class="form-control" id="unique_id"  >
											 </div>
											 
											  <div class="form-group" style="padding:0px">
												<select class="form-control" name="name_type" id="name_type" required >
													<option <?php if(set_value('name_type')=='Mr'){ echo "selected"; } ?> value="Mr">Mr</option>
													<option <?php if(set_value('name_type')=='Mrs'){ echo "selected"; } ?> value="Mrs">Mrs</option>
													<option <?php if(set_value('name_type')=='Miss'){ echo "selected"; } ?> value="Miss" >Miss</option>
													<option <?php if(set_value('name_type')=='Dr'){ echo "selected"; } ?> value="Dr" >Dr.</option>
													<option <?php if(set_value('name_type')=='M/S'){ echo "selected"; } ?> value="M/S" >M/S</option>
												</select> 
											  </div>  
											 
											  <div class="form-group" style="padding:0px">
												 <input type="text" required  class="form-control"  placeholder="Your Full Name"  name="name" value="<?php echo set_value('name'); ?>" class="form-control"    >
											 </div>  
											  ...
											  <div class="form-group" id="sponser_id_div"> 
												 <input type="email" placeholder="Your Email ID"  name="email" value="<?php echo set_value('email'); ?>" class="form-control" id="email"  >
											 </div>  
											  <div class="form-group"  > 
												 <input type="text" required placeholder="Your Mobile No"  name="mobile" value="<?php echo set_value('mobile'); ?>" class="form-control" id="mobile"  >
											 </div> 
											  <div class="form-group"  > 
												 <input type="password" required placeholder="Password"  name="password" value="<?php echo set_value('password'); ?>" class="form-control" id="password"  >
											 </div> 
											  <div class="form-group"  > 
												 <input type="password" required placeholder="Confirm Password"  name="password1" value="<?php echo set_value('password1'); ?>" class="form-control" id="password1"  >
											 </div>
										 ...
										 <button type="submit" class="btn btn-default" id="submit_reg_bt" name="register" value="register1" style="margin-bottom:20px">Register</button>
										<?php } ?>
								 <?php echo form_close(); ?>
						</div>
					 </div>
